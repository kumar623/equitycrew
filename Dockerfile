...
# Only AFTER the model is cached above — set earlier, this flag would block the
# very download it exists to make unnecessary. At runtime it stops the loader
# calling the Hub, which cost ~50s in retries whenever that call timed out.
